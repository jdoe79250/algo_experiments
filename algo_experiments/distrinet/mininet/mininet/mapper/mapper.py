...
# Mapper Switch Bin from Mininet cluster edition adapted to this Mapper
class SwitchBinMapper(DummyMapper):
    def __init__(self, virtual_topo, physical_topo=[]):
        self.physical = physical_topo
        self.vNodes = virtual_topo.hosts()+virtual_topo.switches()
        self.links = virtual_topo.links()
        self.switches = virtual_topo.switches()
        self.servers = physical_topo
        self.controllers = []
        self.servdict = dict(enumerate(self.physical))
        self.hset = frozenset(virtual_topo.hosts())
        self.sset = frozenset(virtual_topo.switches())
        self.hosts = virtual_topo.hosts()
        self.cset = None
        # Server and switch placement indices
        self.places = self.calculatePlacement()
...
class MaxinetMapper(DummyMapper):
    def __init__(self, virtual_topo, physical_topo=[]):
        share_path = "/vagrant/algo_experiments/distrinet/mininet/mininet/mapper/shares/equal_2.txt"
        self.physical = physical_topo
        ...
        metis_name_mapping = {physical_names_mapping[x]: x for x in sorted(physical_names_mapping.keys(), reverse=True)}
        print("metis_name_mapping", metis_name_mapping)
